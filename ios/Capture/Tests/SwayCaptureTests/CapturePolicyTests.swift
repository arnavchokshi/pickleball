...
final class CapturePolicyTests: XCTestCase {
    func testSwingModeUsesLocked1080p120HEVCPolicy() {
        let policy = CapturePolicy.recommended(for: .swing120, deviceTier: .standard, capabilities: .allCodecs)
    ...
    func testLandscapePolicyAcceptsLandscapeAndRejectsPortrait() {
        ...
        XCTAssertNil(CaptureOrientationPolicy.rotationAngleDegrees(for: .portrait))
        XCTAssertNil(CaptureOrientationPolicy.rotationAngleDegrees(for: .portraitUpsideDown))
    }
}
