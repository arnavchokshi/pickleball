...
|---|---|---|---|
| Capture/iOS | AVFoundation, ARKit/manual sidecar, CoreMotion, Core ML/Vision candidates | `ios/Capture`, `ios/Calibration`, `ios/FastTier`, `ios/Guidance`, `ios/Upload`, `ios/Replay` | Swift and strict Python v1 sidecars are currently incompatible, and production upload calls are unwired. Tests/device launch remain scoped until a physical upload/replay vertical slice passes. |
...
| BODY | SAM-3D-Body production path, frame scheduling, remote GPU dispatch, mesh index export, grounding refinement | `scripts/racketsport/remote_body_dispatch.py`, `run_sam3dbody_*`, `threed/racketsport/body_*`, `process_video_body_frames.py` | Normalize the ambiguous production/challenger names in `best_stack.json`. Candidate-label review is not independent GT; the separately tested Fast-SAM-3D-Body challenger is NOT-ADOPT. |
...
| Global fusion | planned robust optimizer over camera/court, players, ball, paddle, contacts and temporal state | roadmap PF tasks; no production stage yet | Raw observations immutable; physically correct surface constraints; covariance/provenance; fixed-anchor/modality ablations; independent GT only. |

## Runtime Notes

- The default full pipeline entrypoint is `scripts/racketsport/process_video.py`.
- It has 19 default serial outcomes. Fresh-run BALL/arc/events currently precede
  BODY/paddle, so same-run wrist/paddle evidence is not fused back. The target
  is an explicit coarse pass, post-BODY refine pass, then global fusion.
- BODY dispatch is remote by default unless `--body-local` or `--no-gpu` is
  used. No universal remote host is valid. Recheck SSH, disk, `nvidia-smi`,
  version stamps and the GPU lock before treating any host as available.
- RTMW/RTMW3D/RTMPose are retired. The offline BODY stack is SAM-3D-Body only:
  do not conflate its registered production label with the separately measured
  Fast-SAM-3D-Body challenger that was slower end-to-end and regressed fast
  swings.
...
- Court auto-find code/candidates exist on main, but corrected owner GT still
  scores the candidates at PCK@5=0. Profiles + guided confirmation are v1; no
  no-tap solver is promoted.
...
- Current artifact reuse is schema/existence based, not dependency
  fingerprinted. Until fixed, use a new run/clip directory for every source;
  `--force` does not replace `source.*`.
