...
INTEGRATION-LANE QUEUE (wave close, ONE lane, fenced files): (1) footattr stage insertion
(body_foot_contact_phases → placement_refine → grounding_refine; regenerated patch from r2 or
re-derive from the verified copy-harness); (2) cammotion camera_motion_auto first-class mismatch
keys per verifier spec; (3) OPTIONAL --remote-host parser-level required=True hardening
(fleethosts follow-up). All via direct fenced-file edits + focused suites; deferred patch files
