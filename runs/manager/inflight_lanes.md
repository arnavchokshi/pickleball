...
| research_trk_rkt_20260716 (Track F, 4 sub-lanes) | Codex gpt-5.6-sol high RESEARCH surveys (read/web-only, no pipeline code): dual independent surveys x2 topics — trk_survey_A/B (NS-03.TRK person det/seg/ReID/mask-cue, what beats RF-DETR ruling + domain adaptation + licenses) and rkt_survey_A/B (NS-03.RKT small/fast 6DoF SOTA + synthetic-data routes + minimal GT capture + gap map); then Track F manager cross-check + 2-vote refutation per eventdata precedent → ADOPTION_REPORTs + benchmark lane specs (spec-only, NO GPU) | Track F manager; codex sessions 019f69f1-cb88-7a20 (trkA pid 56427), -7550 (trkB 56429), -7fd0 (rktA 56431), -70d3 (rktB 56433); full ids + resume template in runs/research_trk_rkt_20260716/MANAGER_NOTES.md | `codex exec [flags] resume <session-id>` per MANAGER_NOTES.md, nohup-detached, flags before resume | runs/research_trk_rkt_20260716/** ONLY (each sub-lane fenced to its own subdir; siblings unreadable by prompt) | — | overnight 07-16; SURVEY.md + livechecks.md per lane, then manager rulings + specs | 2026-07-16 ~02:0x |
...
| ios_recordvis_20260716 | Codex gpt-5.6-sol high (Track D wave 2): blocked states impossible to miss on Record tab — persistent portrait rotate prompt (pre-tap), visible reaction on EVERY tap (motion-layer wobble + banner pulse + haptic, reduced-motion fallbacks), kill the .disabled dead-zone (always-hittable button), banner prominence + VoiceOver announcements; failing-first tests; wave-1 regressions must stay green | codex session 019f69f3-2c1d-78a2-b0a5-9e5b6dd994e6 (PID file runs/lanes/ios_recordvis_20260716/codex.pid) | `codex exec resume 019f69f3-2c1d-78a2-b0a5-9e5b6dd994e6` (nohup fire-and-forget) | ios/App/**, ios/Capture/**, ios/AppTests/**, runs/lanes/ios_recordvis_20260716/** | — | same session: report.json (+MANAGER_VERIFY2.md for sim-blocked steps) | 2026-07-16 ~01:1x |
