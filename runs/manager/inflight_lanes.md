...
| trk_detbench_20260716 | Sonnet GPU-ops (Track F, OWNER-DIRECTED): execute benchmark_spec_trk.md zero-shot arms — arm0a frozen-pool baseline reproduction (0.0001 bar), arm0b BOTSORT-feeder confound check, RF-DETR-L, RF-DETR-Seg-L (+mask archive), D-FINE-L e25 + DEIMv2-L controls (45-min no-attempt budgets); frozen scorer/GT, margin1.0+OSNet frozen; $15 cap / 3.5h wall / on-VM shutdown rail VERIFIED-armed / DELETE+list-confirm; diagnostic only VERIFIED=0. ATTEMPT 1 (H100-only ladder): typed NO-ATTEMPT — 6/6 zones ZONE_RESOURCE_POOL_EXHAUSTED, $0, 0 orphans (list+disks confirmed), auth LIVE (ledger's dead-auth note was stale, corrected), Mac preflight all pins matched. AMENDMENT 1 (manager): SKU fallback per standing ladder — 2 quick H100 attempts then A100-80GB a2-ultragpu-1g then A100-40GB; all rails unchanged (A100 spot cheaper). ATTEMPT 2 dispatched via same agent (context intact) | Track F manager, Sonnet bg agent (resumed) | spec runs/lanes/trk_detbench_20260716/spec.md (see AMENDMENT 1); report.json + DECISION_TABLE.md updated in place | runs/lanes/trk_detbench_20260716/** + gpu_fleet.md rows ONLY (no pipeline code, no commits) | pickleball-{h100,a100}-detbench (attempt-2 provisioning; Track G holds slot 2) | ~2-3.5h from RUNNING | 2026-07-16 morning |
...
| event_head_pretrain_20260716 | Sonnet GPU-ops bg agent (Track G2, DISPATCHED ~09:47): public-data pretrain (jhong93/spot primary + OpenTT, manifest train split only) → pbvision demo anchor inference (R&D reference, NEVER training) → pull w/ two-sided md5. $10 HARD cap ($15 relay not honorable), ≤$5/hr, reuse-first pickleball-a100-fleet1 (STOP at end, disk preserved) fallback fresh spot L4 usc1 (DELETE+confirm); +330 shutdown rail + transfer-aware idle watchdog BEFORE data moves; CODE_GREEN gate w/ md5 pins before code ships; permission denials = verbatim STOP lines in status.log; breadcrumbs runs/lanes/event_head_pretrain_20260716/status.log | Track G2 manager, Sonnet bg agent | status.log + report.json in lane dir | runs/lanes/event_head_pretrain_20260716/** + fleet row (manager-written) | reuse a100-fleet1 or fresh pickleball-l4-eventhead | ~3-4h wall; Mac-side protected-seed + public-holdout eval by manager after ckpt pull; anchor handoff to Track A same afternoon | 2026-07-16 ~09:47 (Track G2) |
