...
|---|---|---|---|---|---|---|---|
| ballarc_scale_guard_20260715 | Codex gpt-5.6-sol high: ball_arc per-segment wall-clock guard (loud typed timeout per trust contract) + segment-7 pool-explosion diagnosis + regression test from pulled real artifacts; CPU-only proof on the salvaged 697s inputs | Track A manager bg b1rjawj55 | report at runs/lanes/ballarc_scale_guard_20260715/report.json when done | threed/racketsport/ball_arc_solver.py + ball_arc_chain.py + its tests + lane dir (fence excludes process_video.py, ball_physics3d.py, timebase files) | none (CPU local) | hours; manager rules on report | 2026-07-16 (coordinator GO, order 1) |
| pbv_harness_v2_20260715 | Codex gpt-5.6-sol high: compare_vs_pbvision_v2 copy-and-fix (full-11-min scipy x0 crash), frozen original byte-identical, regression A (frozen numbers) + B (full-scale EXIT 0 x2 deterministic); gates any 3rd MOVE-1 attempt | Track A manager bg bs7v1lnvu | report at runs/lanes/pbv_harness_v2_20260715/report.json when done | runs/lanes/pbv_harness_v2_20260715/** ONLY | none (CPU local) | hours; manager rules on report | 2026-07-16 (coordinator GO, order 2) |

_(2026-07-16 Track A manager: owner CAL-seed ask STAGED (not executed) at
runs/lanes/pbv11_headtohead_20260713/rerun_20260715/owner_cal_seed/ (2-min tap flow: tap_corners.html
+ chosen frame t=10s + OWNER_CAL_SEED_ASK.md). Deliberately NOT added to OWNER_CHECKIN.md yet —
waiting for Track C's doc-only window-close commit to land first (coordinator sequencing); whoever
closes that window: add this as an owner ask. HARD RULE standing: NO third MOVE-1 GPU attempt
...
| owner_event_labels_20260715 | Codex sol-high (Track E): scaled owner-labeling channel after 29/50 spot-check REJECTED the auto-labeler — tested session generator (stratified sampler audio-onset/track-discontinuity/uniform-random, seed 20260715, 50 reviewed rows + pbvision + protected eval hard-excluded) + half-speed clip pack renderer + results-ingest CLI; stages ~300-clip owner session tonight | Track E manager session 2026-07-15 | re-dispatch from runs/lanes/owner_event_labels_20260715/spec.md; pin d74897203 | NEW scripts/racketsport/build_event_review_session.py + ingest_event_review_results.py + tests/racketsport/test_event_review_session.py (+optional test_event_review_ingest.py) + runs/lanes/owner_event_labels_20260715/** | — | same session: manifest + rendered pack + ingest README + report.json; manager moves pack to ~/Desktop/event_labels_20260715/ | 2026-07-15 |
