...
- Assets: `34`
- States: `BLOCKED=10`, `CONSUMED=7`, `DEFERRED_WITH_REASON=2`, `QUARANTINED=5`, `READY=3`, `REJECTED=7`
...
| `court_labelpack3_owner_labels_20260723` | `READY` | Track A: Fine-tune the current selected court checkpoint on the union of curated264 and these 106 owner-reviewed rows, then compare every epoch on the same exact-semantic Task 88 diagnostic card; keep unsupported negatives out of the current v2 keypoint trainer. | 659483 | 122 owner_decided_frames_including_16_unsupported | 122 package_frames_after_zero-near-match_selection | 106 trainer_loadable_supported_frames_46_new_source_plus_60_owner_label_upgrades | 1446 non_null_owner_keypoints_with_109_explicit_and_35_omitted_invisible_points | human_gt | COURT owner-label data owner | Record the exact training recipe and same-gate before/after metrics; do not double-count the 60 source-overlap rows as new pixels because they are owner-label upgrades. |
...
| `roboflow_court_keypoints_adapted_20260723` | `READY` | Track A: Use this 264-row pack3-disjoint curated subset in the next court fine-tune instead of adding it to the duplicate-heavy 2833-row parent pool; keep net gradients null. | 1791358 | 2844 projective_sanity_pass_keypoint_images_before_dedup | 264 source_and_perceptual_deduplicated_venue_capped_images | 264 trainer_loadable_images | 3168 non_null_floor_keypoints | human_gt | COURT public-data owner | Record the exact fine-tune recipe and same-gate Task 88 comparison; do not claim these rows are new pixels because they are a curated subset of the prior 2833-row external corpus. |
...
### `court_labelpack3_owner_labels_20260723`

- State reason: ready_for_named_consumer
- Paths: `runs/lanes/court_labelpack3_owner_ingest_20260723/train`, `runs/lanes/court_labelpack3_owner_ingest_20260723/source_results.json`, `runs/lanes/court_labelpack3_owner_ingest_20260723/unsupported_view_manifest.json`, `runs/lanes/court_labelpack3_owner_ingest_20260723/ingest_report.json`
- Source families: acmai__pickleball-courts-emwra__v3, chetan-rajagiri-9abfm__pickleball-court-v2__v1, luiss-workspace-99bfi__pickleball-court-detection-o8i4o__v1, necromancer__pickleball-court-vbmkq__v2, nigh-workspace__pickleball-court-vhpgp__v11, testworkspace-i8nb1__pickle-court-keypoints__v2, xuann-bacc-ujr91__pickle-court-keypoints-nluo7__v10
- Partition: train=['44 apparent supported venue groups']; val=[]; test=[]
- Overlap coverage: PASS — all owner-pack frames; zero protected identity overlaps, zero within-pack near matches, and zero near matches against prior reviewed court, labelpack2, or pb.vision labeled sets
- Immutable clean-subset selectors: 0
- Consumers: 0
- License FYI: Original workspace identifiers remain in the package and ingest provenance; technical owner-review and overlap policies determine consumption.
- Disposition: Track A — Fine-tune the current selected court checkpoint on the union of curated264 and these 106 owner-reviewed rows, then compare every epoch on the same exact-semantic Task 88 diagnostic card; keep unsupported negatives out of the current v2 keypoint trainer. (evidence: `runs/lanes/court_labelpack3_owner_ingest_20260723/ingest_report.json`)
...
- State reason: ready_for_named_consumer
- Paths: `runs/lanes/roboflow_court_adapter_20260723/adapted_corpus`, `runs/lanes/roboflow_court_adapter_20260723/adapted_labels.jsonl`, `runs/lanes/roboflow_court_adapter_20260723/report.json`
- Source families: chetan-rajagiri-9abfm__pickleball-court-v2__v1, n-do-tran__pickleball-court-p3chl__v4, necromancer__pickleball-court-vbmkq__v2, nigh-workspace__pickleball-court-vhpgp__v11, pickleball-ball-detection__pickleball-court-keypoints-syncz__v6, ping-pong-paddle-ai-with-images__pickleball-court-p3chl-7tufp__v3, stump-detection-front-view-mj39q__pickle-ball-court-keypoints__v1
- Partition: train=['chetan-rajagiri-9abfm__pickleball-court-v2__v1', 'n-do-tran__pickleball-court-p3chl__v4', 'necromancer__pickleball-court-vbmkq__v2', 'nigh-workspace__pickleball-court-vhpgp__v11', 'pickleball-ball-detection__pickleball-court-keypoints-syncz__v6', 'ping-pong-paddle-ai-with-images__pickleball-court-p3chl-7tufp__v3', 'stump-detection-front-view-mj39q__pickle-ball-court-keypoints__v1']; val=[]; test=[]
- Overlap coverage: PASS — all projective-sanity-pass source candidates; 1392 pack3-near candidates were excluded before dedup and all 264 selected rows have zero protected or owner-pack collisions
- Immutable clean-subset selectors: 0
- Consumers: 0
- License FYI: Source license strings are preserved by the adapter for attribution; the technical floor-only and overlap gates determine this training ruling.
- Disposition: Track A — Use this 264-row pack3-disjoint curated subset in the next court fine-tune instead of adding it to the duplicate-heavy 2833-row parent pool; keep net gradients null. (evidence: `runs/lanes/roboflow_court_adapter_20260723/report.json`)
