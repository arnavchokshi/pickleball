...
| `roboflow_court_keypoints_adapted_20260723` | `CONSUMED` | Track A: Retain these 264 rows in the next structured-v3 training corpus; do not double-count them as new pixels or use them as independent truth. | 1791358 | 2844 projective_sanity_pass_keypoint_images_before_dedup | 264 source_and_perceptual_deduplicated_venue_capped_images | 264 trainer_loadable_images | 3168 non_null_floor_keypoints | human_gt | COURT public-data owner | Use the source groups for future grouped splits and preserve floor-only external supervision. |
...
- State reason: All 106 supported rows were loaded by the named A100 pack3 fine-tune; the 16 unsupported negatives remained excluded from v2 as declared.
...
- Disposition: Track A — Retain the 106 supported rows for structured-v3 training and use the 16 rejected views when the supported-view classifier is actually trained. (evidence: `runs/court_structured_v3_20260723/pack3_selection_summary.json`)
...
- State reason: All 264 rows were loaded by the named A100 pack3 fine-tune; exact input hashes and structured before/after evidence are preserved.
...
- Disposition: Track A — Retain these 264 rows in the next structured-v3 training corpus; do not double-count them as new pixels or use them as independent truth. (evidence: `runs/court_structured_v3_20260723/pack3_selection_summary.json`)
