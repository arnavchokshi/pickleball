#!/bin/bash
# Wave-6 manager watchdog — READ-ONLY early-warning loop (never mutates anything).
...
while true; do
  ...
  for VM in $EXPECTED_VMS; do
    ...
    if [ "$CR_EPOCH" -gt 0 ] && [ $(( ($(now) - CR_EPOCH) / 3600 )) -ge $VM_MAX_AGE_HOURS ]; then
      anomaly B "$VM age exceeds ${VM_MAX_AGE_HOURS}h budget (created $CREATED) — idle-burn suspect"
    fi
  done
  ...
  if [ -n "$BC" ]; then
    echo "$BC" | grep -q "WAVE-5 COMPLETE" || anomaly E "origin BUILD_CHECKLIST lost the [WAVE-5 COMPLETE] bullet"
  fi
