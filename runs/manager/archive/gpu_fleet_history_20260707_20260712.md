# GPU fleet ledger — verbatim history through 2026-07-12

Archived 2026-07-13 from runs/manager/gpu_fleet.md (doc/org session). Every row and wave log
below is preserved verbatim; the live ledger keeps only current state + standing policy.
...
| pickleball-h100-trka | asia-southeast1-b | H100-80GB (a3-highgpu-1g) | SPOT | DONE+DELETED 2026-07-12T20:27Z list-confirmed absent (created 2026-07-12T18:47:43Z, first-attempt asia-southeast1-b, zero preemptions, uptime 1.655h ~= $0.94-7.03 mid ~$2-3.5) | trk_reid_apron_20260712 | ~$0.57-4.25/hr quoted, <=$5/hr cap held | 2026-07-12T18:47:43Z | Slot A of sprint PLAN §3 rank-6 TRK ReID/apron. PREFLIGHT: code pinned+checked-out to EXACTLY 3d5125d58 via git bundle (main had advanced to a57476a39 concurrently on this same repo -- pin honored, not main tip); scorer md5 byte-verified 3/3; OSNet sha256 2809d3227f7d... byte-exact match, loader+embedding smoke PASS (real torchreid_osnet_x1_0, 512-d, no clip-training leakage); denylist scan clean twice (pre-copy + pre-score). RESULT: frozen 4-arm matrix (loose_pool/margin_0.5/1.0/2.0) run + scored on Burlington/Wolverine historical-internal GT via benchmark_person_trackers.py + run_raw_pool_person_authority.py (OSNet-margin embeddings computed once, reused across margin arms) + score_person_track_sources.py -- PARTIAL SURVIVE: margin_0.5 and margin_1.0 both improve worst-clip IDF1 (0.6425->0.8259/0.8516) and worst cov4 (0.0433->0.6133/0.7117) over loose_pool control with zero new switches/true-spectator-FP/far-off-court-FP; margin_2.0 REJECTED (introduces 1 new ID switch + 24 new far-off-court FP frames on wolverine). HARVEST: found genuine schema-incompatibility blocker (harvest court_calibration.json = racketsport_harvest_source_court_calibration schema, not the canonical CourtCalibration the harness requires) -- blocks calibration-dependent margin/embedding-coverage diagnostics for all 6 zero-label clips (STOP+report per rule, not patched); ran calibration-free raw-detector->=4-rate + raw BoT-SORT track-longevity/fragmentation diagnostics instead (reuses harness detection code unmodified) on all 6 full-length clips (2736-29130 frames each). Weight license unresolved -> R&D-only, VERIFIED=0, no promotion, no best-stack edit. Evidence: runs/lanes/trk_reid_apron_20260712/ (vm_pull/ + report.json, both-sides sha256). |

WAVE-BODY FLEET LOG (2026-07-12, body_overhead_20260712 lane):
- pickleball-h100-bodyc (H100 a3-highgpu-1g SPOT, ase1-b/-c, usc1-a/-b, euw4-b ladder, pd-balanced
  200GB FROM pickleball-fleet-snap-20260709-w7close, 120s inter-attempt backoff, 6-attempts/30min
  cap) — PROVISIONING (Sonnet lane body_overhead_20260712, self-tearing, wall cap 5h). GPU Slot C
  of sprint PLAN §3 rank-6 BODY overhead. PREFLIGHT: base code pinned to committed main
  5672f248659b45d05f3f154aa36276285b264cb7 via git bundle; lane's OWN exclusive-fence diff (5
  files: remote_body_dispatch.py, run_sam3dbody_batch.py, NEW sam3dbody_persistent_worker.py,
  test_remote_body_dispatch.py, NEW test_sam3dbody_persistent_worker.py) shipped as a separate
  tar overlay on top of the clean bundle checkout (both-sides sha256 pinned pre-transfer,
  concurrent sessions' unrelated dirty tracked files excluded/not shipped); 149/149 owned+adjacent
  local tests green pre-provision (29+76+16+3 new +9 cli-help via .venv); dead-code audit clean;
  scaffold-tool audit resolved for the new CLI (pre-existing unrelated "unknown" category failure
  attributed to a DIFFERENT concurrent session's uncommitted scripts/racketsport/
  run_court_temporal_lock.py, not touched); denylist scan clean (Wolverine-only, no Outdoor/Indoor
  identifiers anywhere in this lane's plan). Concurrency at provision time: 0/5 RUNNING (fleet1
  TERMINATED, trkA already DONE+DELETED). Mission: same-VM cold->warm Wolverine full-stack baseline
  (must reproduce banked 244 BODY frames / 705 player-frames) -> Lever A persistent model/compile
  worker -> Lever B compile-cache persistence across runs -> Lever C benchmark --body-schedule
  overlap (no default change) -> combine only individually-surviving levers; >=15% full-pipeline
  AND BODY-wall improvement bar with frozen metric parity, peak RSS <=+15%. VERIFIED=0; PENDING-only
  best-stack delta if any lever survives; no promotion.
- pickleball-h100-bodyc -- DONE+DELETED 2026-07-12T22:17:50Z list-confirmed absent (created
  2026-07-12T20:55:40Z first-attempt asia-southeast1-b, zero preemptions, uptime 1.37h ~= $0.78-5.82).
  ALL THREE LEVERS REJECTED against the pre-registered >=15% full-pipeline-wall AND BODY-wall bar
  (matched same-VM rows, banked 244 BODY frames / 705 player-frames reproduced exactly, gate
  passed/coverage 1.0 on every valid arm): Lever A (persistent model/compile worker, NEW
  sam3dbody_persistent_worker.py) genuinely works (model_load_s+compile_warmup_s 0.0/0.0 on the
  served job, fingerprint-matched, gate/joint/grounding parity exact vs baseline) but clears only
  the BODY-wall bar (248.612s->205.913s, -17.17%) while missing the full-pipeline bar (487.101s->
  442.064s, -9.25%) because BODY is no longer wall-dominant (the "world" stage has grown to
  ~122s/run since the w7speed-era baseline, apparently from later spine/facts landings -- flagged,
  not this lane's fence); Lever B (compile-cache persistence) measured a genuine 0% incremental
  gain -- torch's default /tmp/torchinductor_<user> on-disk cache is ALREADY warm/reused across
  separate subprocess invocations within one VM session (25.59s default-repeat vs 25.85s
  explicit-named-persistent-dir-repeat, within noise; both far above the impossible-to-cache
  Python/dynamo-tracing floor); Lever C (--body-schedule overlap, existing flag, no default
  touched) showed an attractive raw wall-clock number (491.459s->369.306s, -24.86%) that is NOT
  valid evidence: it silently drops the banked workload (705->641 player-frames, 244->222 scheduled
  BODY frames) because BODY dispatch starts before ball/event-derived deep-mesh-window selection
  completes in overlap mode, and the full-clip gate FAILS (coverage 0.909 < 0.98) -- correctly
  killed as workload/schedule drift, not a real like-for-like speedup. No lever combined (none
  individually survived). BEST-STACK DELTA: none (nothing survived to promote to PENDING).
  VERIFIED=0 unchanged. Code landed (default-off, opt-in via SAM3DBODY_PERSISTENT_WORKER_SOCKET /
  RemoteConfig.sam3dbody_persistent_worker_socket env-var toggle only) committed 32d6db3c2 (already
  swept into an origin push by a concurrent session); 149/149 owned+adjacent local tests green
  pre-provision, 46/46 re-verified on the VM. Pre-existing template/repo gaps found+worked around,
  not fixed (out of fence): w7close snapshot lacked the OSNet ReID checkpoint newly required by
  best_stack rev 11 (transferred from Mac, sha256 byte-verified, matches trkA's same finding);
  self-dispatch SSH to the VM's own external IP requires a fresh ssh-keyscan self-entry in
  configs/ssh/a100_known_hosts AFTER every git checkout/reset (tracked file, gets overwritten --
  new gotcha for the boot-ritual checklist); 2 pre-existing test_remote_body_dispatch.py tests fail
  on this fresh VM only because bare `python` (vs `python3`) isn't on PATH in a localhost-SSH test
  fixture, unrelated to any lane's code (passes clean on Mac). Evidence:
  runs/lanes/body_overhead_20260712/ (vm_pull/ + report.json, both-sides sha256 for lane-owned files).
