# In-flight lanes — verbatim history through 2026-07-12

Archived 2026-07-13 from runs/manager/inflight_lanes.md (doc/org session). All lane rows and
dated session notes below are preserved verbatim (all lanes DONE/closed except BL-E
ball_anchor_boost_20260712, which remains in the live file).
...
_(2026-07-12 ~19:0x: BODY-OVERHEAD SLOT C DONE — all 3 levers REJECTED vs >=15% dual bar (honest):
A persistent-worker real+working (-17.2% BODY) but full-pipeline only -9.25% BECAUSE of a NEW FIND:
world stage regressed ~1s->~122s since w7speed (outside lane fence) — now dominates wall more than
BODY; B compile-cache = null lever (already persistent); C overlap-schedule = INVALID (drops 705->641
player-frames, coverage 0.909 — schedule-drift kill worked). Worker code committed by lane 32d6db3c2
(version-stamp verifier requires committed tree), default-off env opt-in. bodyc VM DELETED
list-confirmed ~1.37h ~$0.78-5.82. GPU WAVE COMPLETE: trkA + bodyC both closed clean; Slot B cancelled;
RF-DETR not triggered; peak concurrency 2/5 mine. WORLD_PERF_20260712 DISPATCHED (codex xhigh):
diagnose+parity-safe-fix the 122s world stage (suspects: rev-11 fail-closed emission / fixp verdict
map / spine017 refined events). Ops finds booked: w7close snapshot missing OSNet (2nd hit — re-bake
at next cut), python-vs-python3 PATH on fresh VMs, post-sync self-keyscan. Concurrent session staged
CVAT court tasks 88-91 + wrote its memory (court harness ours 6.61px vs pb 5.67px).)_

_(2026-07-12 ~19:2x: WORLD_PERF RULED PASS — 122s 'regression' ATTRIBUTED, not a bug: spine017's
post-BODY refined-events pass (7fab3804c) runs the refined BALL arc solve inside the world stage
(327/328s profile = refined arc work; world assembly 0.65s; dependency-current reuse 0.636s; parity
byte-identical). Intended NS-01.7 computation. BOOKED as top NS-06 lever: incremental/reuse-aware
refined solve; ALSO un-caps body_overhead lever A (persistent worker -17.2% BODY failed only the
full-pipeline bar because of this stage — re-baseline after the reuse lever). No source change; no
commit. Remaining live: BL-E ball_anchor_boost only. Close-out begins.)_

_(2026-07-13 ~00:2x: OWNER BACK + supplied 11-MIN pb.vision export set (banked
data/pbvision_11min_20260713: cv 12.4MB sha 4ccd08fd, insights 41 rallies sha fbe3e162, stats sha
09a6da41 — same schema versions as banked sample; ONE camera segment across 20,922 frames).
OWNER RULINGS: (1) OSNet license fine internal/private -> TRK margin candidate gate-blocked only
(recorded 13c325c77; flip-to-preview offered, awaiting word); (2) single OWNER_CHECKIN.md now
canonical (082326dee/825729032, dated ones deleted). DUAL-MODEL DEEP REVIEW RUNNING per owner
directive: [a] pbv11_forensics_20260713 codex sol-xhigh (7-question quantitative spec incl. product-
surface map + reproduction re-ranking); [b] Fable workflow wf_7e3c1be5-e38 (5 independent lenses +
adversarial re-computation verify); [c] Sonnet video hunt (source mp4 for same-clip head-to-head —
GPU run of OUR stack on the 11 min if obtained). BL-E was WEDGED (codex client model-manager timeout,
0% CPU, never started) -> killed + redispatched pid 26728. Synthesis -> exact task plan -> immediate
execution when streams land.)_
