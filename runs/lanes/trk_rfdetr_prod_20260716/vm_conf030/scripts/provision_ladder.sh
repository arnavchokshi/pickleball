#!/usr/bin/env bash
# vm_conf030 micro provisioning ladder — A100 tiers only (per coordinator: skip H100). 120s backoff.
set -u
...
LOG="$ROOT/runs/lanes/trk_rfdetr_prod_20260716/vm_conf030/logs/provision_attempts.log"
VM="pickleball-gpu-conf030"
...
for entry in "${LADDER[@]}"; do
  ...
  if [ $rc -eq 0 ]; then
    echo "SUCCESS attempt=$n tier=$tier mt=$mt zone=$zone"; echo "$out"; exit 0
  else
