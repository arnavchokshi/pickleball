...
V2 preserves the frozen single-rally calculations while fail-closing extreme
physics-fit seeds as typed, counted skips.  A full export can contain a short
segment whose exported 3D positions imply an initial velocity outside the
optimizer's documented bounds; fitting it after silently clipping the seed
would turn an implausible competitor-reference segment into a plausible-looking
result.  V2 therefore records the segment and its diagnostics without fitting.
...
def physics_fit(samples: Sequence[Sample], segment_index: int) -> dict[str, Any]:
    ...
    lower_bounds = np.asarray([-60.0, -60.0, -60.0, 0.0])
    upper_bounds = np.asarray([60.0, 60.0, 60.0, 1.0])
    initial_guess = np.concatenate([velocity_seed, [0.01]])
    outside_bounds = np.flatnonzero((initial_guess < lower_bounds) | (initial_guess > upper_bounds))
    if outside_bounds.size:
        axis_names = ("vx", "vy", "vz", "drag_k")
        return {
            ...
            "status": "physics_fit_skipped",
            "physics_fit_skipped_reason": "initial_guess_outside_optimizer_bounds",
            "initial_velocity_seed_mps": [float(value) for value in velocity_seed],
            "initial_velocity_seed_speed_mps": float(np.linalg.norm(velocity_seed)),
            "optimizer_velocity_bounds_mps": [-60.0, 60.0],
            "out_of_bounds_parameters": [axis_names[int(index)] for index in outside_bounds],
            "observed_z_range_m": [float(points[:, 2].min()), float(points[:, 2].max())],
        }
...
def physics_pillar(trajectory: Trajectory) -> dict[str, Any]:
    ...
    skipped = [row for row in rows if row.get("status") == "physics_fit_skipped"]
    # Keep byte-for-byte frozen behavior when no new typed skip occurs while
    # making full-export drops impossible to miss in the aggregate scorecard.
    if skipped:
        result["physics_fit_skipped_count"] = len(skipped)
        result["physics_fit_skipped_reason_counts"] = dict(
            sorted(Counter(row["physics_fit_skipped_reason"] for row in skipped).items())
        )
    return result
