...
**Verify-before-commit (bleeding-edge, some possibly future-dated in source research):** Multi-HMR 2, SAT-HMR, WATCH, OnlineHMR, HTD-Refine, PhysHMR, CalTennis, BEDLAM2, TT4D — confirm repo/license/benchmarks resolve before relying on them. **Current build-on stack:** SAM-3D-Body + Fast SAM-3D-Body, YOLO26, BoT-SORT/ByteTrack, RTMPose/RTMW, MotionBERT, TrackNetV3, PhysPT, PHC/PULSE, MuJoCo+MJX, FoundationPose/GigaPose/FoundPose, OpenCV PnP, Three.js. Individual components still require the repo's stage-specific runtime, license, and accuracy gates before product claims. Established fallbacks: SAM-3D-Body per-crop (for Multi-HMR 2/SAT-HMR preview), PhysPT (for PHC), RF-DETR-L/RTMDet (for YOLO26).
