...
### (c) Person detection + tracking + doubles ID — **YOLO26m + BoT-SORT-ReID + court geometry** *(round-4b target; runner registered, gate pending)*
...
### (p) Target: world-grounding + foot-skate elimination — **SAM-3D-Body per-frame → court-plane grounding → contact-vs-Z=0 + CCD-IK** *(round-4b)*

- **Chosen target:** take the **Fast SAM-3D-Body** per-frame mesh (e) and ground it through the known court frame — place the root in world via known K,[R|t] + court plane Z=0 (no SLAM, no estimated gravity; precedent arXiv 2512.21573), temporally smooth, then the **foot-skate killer**: detect contact against the **known court plane** (foot height-above-Z=0 < ~2–3 cm AND world-speed < ~1 cm/frame AND pose-confidence, with hysteresis) → **snap stance toe/heel to Z=0 + zero world velocity → CCD-IK** so the leg meets the locked foot while the upper body keeps its motion. UnderPressure learned contact classifier if thresholds prove noisy in fast lunges. **GVHMR/WHAM are an optional cross-check on the world trajectory only — not the mesh.**
- **Target advantage after Phase 3/4 gates:** SAM-3D-Body should give a better per-frame mesh than the world-HMR methods, and the calibrated court plane should make the ground constraint stronger. The **~2–3 mm foot-slide or below** target and zero-floor-penetration claim remain gated until BODY accuracy, foot-contact, and physics/replay validation pass. Build/test in `IMPLEMENTATION_PHASES.md` Phases 3–4.
...
**Verify-before-commit (bleeding-edge, some possibly future-dated in source research):** Multi-HMR 2, SAT-HMR, WATCH, OnlineHMR, HTD-Refine, PhysHMR, CalTennis, BEDLAM2, TT4D — confirm repo/license/benchmarks resolve before relying on them. **Current runtime evidence is narrower than the target stack:** repo-backed runtime work currently centers on SAM-3D-Body/Fast SAM-3D-Body smoke paths, YOLO26/BoT-SORT/ByteTrack tracking work, TrackNetV3/V4/TOTNet/PBMAT adapters, OpenCV PnP, CPU physics/replay scaffolds, and the review-only Three.js surface. PhysPT, PHC/PULSE, MuJoCo+MJX simulation, and FoundationPose/GigaPose/FoundPose remain candidate/probe targets until their readiness gates and registered runtime paths pass. Individual components still require the repo's stage-specific runtime, license, and accuracy gates before product claims. Established fallbacks: SAM-3D-Body per-crop (for Multi-HMR 2/SAT-HMR preview), PhysPT (for PHC), RF-DETR-L/RTMDet (for YOLO26).
