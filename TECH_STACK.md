...
| Ball/events | WASB/TrackNet family, audio onset, bounce/in-out, physics fill, event fusion | `threed/racketsport/ball_*`, `event_fusion.py`, `rally_gating.py`, `scripts/racketsport/build_*ball*`, `process_video.py` | Confidence-gated preview only until reviewed F1/contact/in-out gates pass. |
| BODY | Fast SAM-3D-Body, frame scheduling, remote A100 dispatch, grounding refinement | `scripts/racketsport/remote_body_dispatch.py`, `run_sam3dbody_*`, `threed/racketsport/body_*`, `process_video_body_frames.py` | Candidate-label review is not independent GT. |
...
| Paddle | detector/segmenter candidates, PnP-IPPE, future reference-pose stack | `threed/racketsport/racket_*`, `paddle_proxy.py`, `scripts/racketsport/rkt_*` | Box-derived candidates cannot become 6DoF claims. |
