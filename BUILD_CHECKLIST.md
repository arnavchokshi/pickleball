...
- [IOS UIFIX 2026-07-07, Codex ios_uifix_20260707] Production UI repair pass
  for `ios/**`: splash uses lens-on-raster eyelids over the owner mark eye
  region with geometry tests and a static preview snapshot; tab chrome uses one
  raised center Record control with no stray spinner and the Record text label
  omitted to avoid collision; screenshot walker fixtures were added for the
  manager visual gate. Evidence/report:
  `runs/lanes/ios_uifix_20260707/`. This is UI/walker proof only; IOS-1 remains
  `SCOPED PASS` and `VERIFIED=0`.

- [IOS BRAND V4 2026-07-07, Codex] iOS app-shell brand pass in `ios/**`:
  center-raised code-drawn textured Record button, five-tab rail with honest
  Coach P6 placeholder, hand-drawn stroke/underline/swoosh motion gates,
  flushed sample Stats/Replays/Profile surfaces, and branded WorldViewer chrome
  with simple presets/speed/follow/coach-mark controls. This is UI/product
  polish only; capture sequencing still goes through the existing
  `CaptureViewModel` setup-pass/record flow, server replay semantics are
  unchanged, and IOS-1 remains `SCOPED PASS` / `VERIFIED=0`.

- [LIVE/OFFLINE DOCS 2026-07-07, Codex] Manager ruling from
  `runs/research_liveondevice_20260707/` landed in the docs of record:
  `CAPABILITIES.md` now owns L0/L1 advisory live, L2 server-fast, and L3 deep
  world naming; `TIER_MAP.md`, `NORTH_STAR_ROADMAP.md`, `EDGE_PLAYBOOK.md`,
  `MASTER_PLAN.md`, and `ios/README.md` mirror that split. Live calls remain
  advisory/uncertainty-banded; L3 remains the only promotion authority;
...
- [P6-1 SHOT RULES DESIGN 2026-07-07, Codex p61_shot_rules_20260707 (stream-4)] ACCEPTED — design-only per TECH_BLUEPRINTS COACHING S1: docs/racketsport/shot_rules_v0.json (9-row first-match table: 8 classes + unknown, blueprint-exact thresholds/confidences, per-row rationale, machine-readable conditions, trust modifiers 0.5/0.3) + threed/racketsport/shot_rules.py (stdlib-pure loader/validator + classify_shot/classify_shots, NO artifact emission, NO pipeline wiring) + tests/racketsport/test_shot_rules.py (18 green: every class fired, null-feature arc-less safety, precedence, modifier, loader-rejection). Anti-dup ruling B.4#10 honored: shot_taxonomy.py remains the ONLY racketsport_shots producer; reconciliation note in-table (volley added; atp/erne/tweener out of v0 scope; serve/return semantic-shape flagged for the future integration lane, which threads load_shot_rules into classify_shots_from_payloads). Wide suite 3074 passed / 0 failed / 18 skipped (2965s). Manager spot-checked table thresholds + module purity + file boundary personally. VERIFIED=0 unchanged.
- [P0-7 PHASE-2 FLIGHT CORPUS 2026-07-07, Codex p07_flight_corpus_20260707 (stream-4)] ACCEPTED — synthetic flight corpus LANDED per BALL 3D STEP 3: runs/flight_corpus_20260707/flight_corpus_train_50000.jsonl (50,000 records, seed 20260707, 3.7GB) + flight_corpus_val_5000.jsonl (5,000, seed 20260708, 382MB); flight-sanity pass 100%/100% (bar >=95%), failed_segments=0 demoted_frames=0; error-profile match vs measured (34px p95 / 0.578 recall / 0.021 hidden-FP): jitter 34.23 (0.7% off), recall 0.5772 (0.14%), hidden-FP 0.0216 (2.7%) — all within_20_percent=true (error_profile_match.json); occlusion-BURST dropout + calibration embed + bounce_params_measured=false tagging in the CLI layer, flight_simulator.py untouched (read-only honored); 6 focused tests + scaffold checker green (BALL/P0-7). MuJoCo prestage also done this stream: mujoco 3.10.0 in .venv, stepping smoke PASS (MJX install script is CUDA-VM-shaped — deferred to VM time as designed). STEP 4 (lift-net) reads exactly these two paths. DISK NOTE: corpus = 4.1GB under runs/ (gitignored); prune candidate if Mac disk tightens. VERIFIED=0 unchanged.
- [P0-4 CORPUS DASHBOARD 2026-07-07, Codex p04_corpus_dashboard_20260707 (stream-4)] ACCEPTED — DATA D6 gate tool LANDED: scripts/racketsport/corpus_dashboard.py aggregates roboflow/harvest/owner_capture/human_review sources (counts per source/role/visibility, diversity, dedup) and RE-RUNS leakage every run via imported assert_no_protected_eval_hash_collisions (35 eval hashes) + reserved-held-out id string-scan (pwxNwFfYQlQ/vQhtz8l6VqU); exit 1 on any hit. Manager personally re-ran the CLI: exit 0, eval_hash_collisions=0, heldout_id_hits=0, roboflow 61,260 / harvest 43 / human boxes 274 / owner_capture honestly present=false (manager_spotcheck.json). Seeded synthetic leakage fixtures CAUGHT exit-1 with named hits in tests (5 green + scaffold 3 green, DATA/P0-4). No real leakage today. VERIFIED=0 unchanged.
- [STREAM-4 WIDE-SUITE ADJUDICATION 2026-07-07, manager] Both p07+p04 wide suites = 3073 passed / 1 failed / 18 skipped; the single failure (both lanes, same test) = test_flight_simulator.py::test_generate_corpus_is_deterministic_and_fast_for_small_cpu_sample, a wall-clock-only assert (<5.0s) on a file NEITHER lane modified, failing under 3 concurrent wide suites (elapsed monotone with load: 23.1s heavy -> 7.5s -> 5.87s -> 5.59s -> 5.03s residual; manager adjudication run missed the bar by 0.6%); the SAME test passed inside p61's concurrent 3074/0 run. RULED: host-contention timing flake (known wave-2 family 'flight-sim load timing'), not a regression. FLAG for BALL pillar owner: the 5.0s threshold has ~0% margin on a loaded Mac — consider headroom or a slow-marker; stream-4 did not touch the fenced file. ALSO: bg-session kill-sweep killed all 3 original codex dispatches ~25min in; detached nohup resume (codex exec resume + self-written report) recovered all 3 with zero work lost — pattern for future bg-session managers. VERIFIED=0 unchanged.
