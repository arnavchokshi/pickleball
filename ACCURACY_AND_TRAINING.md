...
> **Round-4 update (mesh is now core; body backbone target = Fast SAM-3D-Body).** The earlier "skeleton-first" choice is **superseded: SMPL/SMPL-X mesh is now the core target body representation**, with the fast skeleton demoted to a preview/triggering overlay. The per-frame mesh backbone target is **Fast SAM-3D-Body** — supported by external benchmark claims (3DPW PA-MPJPE ~30.4 mm, beats HMR2.0 by 15+ mm) and user hands-on checks, and it keeps the existing `sam4dbody` investment. Current repo evidence is scheduled-frame BODY smoke, not BODY verification. (License: SAM License — **fine for research/personal use now; verify before commercial**; the license-safe Apache fallback is **SAT-HMR** — see the round-7 note and `TECH_STACK.md §2.3`.) **World-grounding/foot-contact is built by us** off the known court plane — we do **not** adopt GVHMR/WHAM as the primary mesh (their per-frame backbone is HMR2.0, which is weaker; demoted to optional trajectory/foot-velocity sanity-checks). **Licensing is still lifted for planning** (not selling yet), but dataset access/license/manifests still need verification before training or product claims. See `TECH_STACK.md` §(e),(p)–(s) and `IMPLEMENTATION_PHASES.md` Phases 3/4/6/10 for the full body/physics/racket/replay design; this doc covers the *training and accuracy* angle.
...
> **2026-06-30 hardware caveat:** current live GPU access is A100 diagnostic/prototype use, not the H100 gate path. A100 training or setup runs can inform decisions only when logged as such; they do not lock variants or promote phase gates unless the checklist row explicitly accepts A100 evidence.
...
- **LiDAR depth (Pro devices, optional bonus):** near-camera (~5 m) depth as *extra* supervision (near-player foot-contact, near court-plane) — **not** court-spanning, **not** the ball, fails in direct sun. Vision-first is the baseline.
...
**Current implementation note (2026-06-30):** the default server calibration stage now attempts automatic semantic court evidence for every available upload video/frame and writes `court_line_evidence.json` with kitchen/NVZ, centerline, and top-net readiness. Video-backed runs stop before tracking when that evidence is not ready. A random video without a trusted calibration seed still fails closed instead of fabricating `court_calibration.json`; the trained/heuristic no-tap solver must pass the gates below before this becomes verified automatic calibration. The current no-tap court-keypoint checkpoint is not promotable: threshold 0.5 yields 0 confident keypoints, and the latest CPU diagnostic at `runs/pickleball_pretraining/court_keypoint_20260628/diagnostic_20260630_cal_lead_verify_current/` shows threshold 0.05 yields correspondences that still fail reprojection/world/temporal validation with 0 diagnostic gate-passed clips; its `overlay_review/` sheets show the predictions are not aligned with the trusted calibration projection.
...
- **SERVER OFFLINE / deep tier — per-frame backbone: Fast SAM-3D-Body** (best per-frame mesh available — 3DPW PA-MPJPE **30.4 mm**, beats HMR2.0 by 15+ mm; user-confirmed; SAM License — research-OK now, verify before commercial, SAT-HMR/Apache is the license-safe fallback; ~15 FPS/person, ~4–6 FPS for 4 batched crops — speeds are estimates, benchmark on accepted GPU hardware). MHR→SMPL via its built-in MLP. **World-grounding is OURS, not GVHMR's:** project per-frame output to world via known K,[R|t] + court Z=0 → temporal smooth → foot-lock (§5c) → physics (precedent arXiv 2512.21573). **GVHMR/WHAM/TRAM/OnlineHMR are demoted to optional world-trajectory / foot-velocity sanity-check references only with the court-plane hard constraint** — their per-frame mesh is HMR2.0-based (weaker), and the world-grounding they add is exactly what our calibration already gives us. **PromptHMR is dropped** (full-image hurts distant players; needs manual prompts; worst translation error on CalTennis).
- **SERVER-FAST preview — SAT-HMR (~24 FPS) or Multi-HMR 2 (~20 FPS)** camera-space multi-person SMPL — **prefer Multi-HMR 2** (no fixed-FOV assumption; SAT-HMR assumes ~60° FOV which our variable cameras violate). ~7–8 mm penalty vs SAM-3D-Body, acceptable for a server preview. This is not ON-DEVICE LIVE. The phone-live tier uses 2D pose/joints only; the positional skeleton (RTMW3D/MotionBERT) is now only a server preview overlay + sub-frame contact trigger.
...
**Pipeline (server offline default):** confidence-gate keypoints → outlier-reject (median, win 3–5) → **court-homography projection to metric world plane** → **RTS/Kalman smoother** → central difference → **4th-order zero-phase Butterworth (`filtfilt`)** at **6 Hz body/timing, 8–10 Hz swing**; or **Savitzky-Golay (order 2, win 9)** which differentiates + smooths and preserves peaks better. **On-device live 2D cues:** One-Euro filter (β≈0.007, f_c_min≈1.0 Hz) — adaptive cutoff preserves the swing peak. Set cutoffs by residual analysis (Winter's method), higher for wrist than trunk.
...
Full design/build in `IMPLEMENTATION_PHASES.md` Phase 6 (`TECH_STACK.md` §(r)). **Rationale:** tracking the paddle directly should beat inferring face angle from the wrist by ~5–15×, but current repo proof is limited to fail-closed PnP/review scaffolds; no accepted-four true-corner/reference/ArUco RKT gate has passed. The 2026-06-30 true-corner review manifest at `runs/cvat_imports/2026_06_30/rkt_true_corner_review_manifest_20260630_goal_continuation/` is a review queue only: it consolidates 755 accepted-four required labels and 2,211 CVAT rectangle-derived candidates, with 0 true-corner/reference labels and no promotion.
...
**Physics post-processing (CPU, real-time):** EKF `[x,y,vx,vy]` with gravity → RANSAC parabola fit (reject >5 px off-arc, fill gaps) → **bounce = vertical-velocity sign change** (sub-frame via parabola interpolation → ±5–15 ms even at 30 fps) → **net crossing via homography** → rough 3D via physics + "Z=0 at bounce." Target, not current evidence: the current repo has not proven this FP reduction, and the 2026-06-30 full-CVAT local-search postprocess was rejected because it regressed dense and hard-negative candidates.
...
**Licensing is informational now, not a sales gate** (we are not selling yet — recorded for future commercialization), but dataset access, local manifests, and license terms still need verification before training or product claims. AMASS / BEDLAM / BEDLAM2 / AthletePose3D / AthleticsPose / SMPL / SMPL-X / CalTennis (and GVHMR/WHAM, used only as trajectory references) are **candidate research/personal-use sources**, not proof that the repo has populated training data today. Body-model **fine-tune/adapt order (round-4b): BEDLAM2 → AthletePose3D → CalTennis → RICH → AMASS** (priors), adapting the **Fast SAM-3D-Body** pipeline; eval world-MPJPE on EMDB2 + CalTennis. ASPset-510 (CC0) and SportsPose remain useful sports sources. **Verify-before-commit** (very recent, confirm repo/license/numbers): CalTennis, BEDLAM2, SAT-HMR, OnlineHMR, WATCH.
...
**TrackNet runtime note (updated 2026-06-30):** TrackNetV3 and InpaintNet checkpoint files are hash-verified. Real smoke runs produce schema-valid `ball_track.json`, and the current A100 diagnostic substrate now has full-CVAT TrackNetV3 outputs for Burlington, Wolverine, and Outdoor 0-1150. This is runtime/horizon evidence only: the raw full-CVAT candidate scores F1@20 0.554 with 61 teleports, so it is not BALL verification. A dense CVAT-to-TrackNet label artifact writes `Frame,Visibility,X,Y` CSVs for 2,051 reviewed frames, preserving 527 explicit hidden negatives for training; the materialized dataset at `runs/ball_tracknet_cvat_dataset_20260630T175124Z_materialized/` and first A100 fine-tune benchmark at `runs/ball_tracknet_finetune_20260630T1812Z/cvat_benchmark/tracknet_dense_hidden_finetune_vs_cvat_20260630T183151Z.json` improve to F1@20 0.626, precision@20 0.606, recall@20 0.647, hidden-FP rate 0.342, and 2 teleports, but still do not pass BALL. The error analysis at `runs/ball_tracknet_finetune_20260630T1812Z/error_analysis_20260630_goal_continuation/` includes annotated `review_sheets/` and `hard_negative_plan/`, showing Burlington/Wolverine hidden-negative failures, teleports, missing visible predictions, and Outdoor localization/recall failures while keeping Outdoor validation-only. That hard-negative plan is now materialized at `runs/ball_tracknet_cvat_dataset_20260630T211745Z_hard_negative_context8_repeat3_materialized/`, but the fresh A100 epoch-20 benchmark at `runs/ball_tracknet_hardneg_finetune_20260630T2130Z/cvat_benchmark/tracknet_hardneg_epoch20_vs_cvat_20260630T2200Z.json` regresses to F1@20 0.569 and held-out Outdoor F1@20 0.296, so this hard-negative-only resume is rejected. The CPU local-search postprocess benchmark at `runs/ball_local_search_postprocess_20260630T225735Z/` is diagnostic-only and rejected too: dense local-search postprocess regresses to F1@20 0.487 / precision 0.429 / recall 0.563 / hidden-FP 0.970 / 10 teleports, and hard-negative local-search postprocess regresses to F1@20 0.502 / precision 0.490 / recall 0.514 / hidden-FP 0.342 / 18 teleports. TrackNet's official CSV exposes `Frame,Visibility,X,Y`, not calibrated probability, so confidence is recorded as visibility-derived `1.0/0.0` until a real heatmap-confidence adapter is built. Phase 5 still requires labeled ball/event ground truth, contact-window generation, and the documented F1/timing gates.
...
2. **On-device court calibration seed/cache + capture-quality guidance** — ARKit intrinsics+pose+floor plane once at setup; everything downstream depends on trusted geometry.
3. **On-device person detect + track + N-lock and 2D pose/joints** — CoreML YOLO26n/s + lightweight ByteTrack/BoT-SORT + court-polygon filter; Apple Vision/CoreML RTMPose-m.
4. **On-device ball heatmap spike + cheap line/contact cues** — distilled/quantized CoreML heatmap tracker (~288p) is the top optimization risk; homography+bounce line calls and mic+wrist contact timing stay fail-closed until it passes.
5. **Server ball (TrackNetV3→V5/PB-MAT) + audio contact model** — enables final event-triggered compute + scene physics.
6. **World-grounded SMPL mesh** — **Fast SAM-3D-Body** per player (best per-frame mesh) + our own grounding to the court plane; sports adapt (BEDLAM2 → AthletePose3D → CalTennis → RICH → AMASS). GVHMR/WHAM/TRAM/OnlineHMR only as trajectory cross-checks with the court-plane hard constraint.
7. **Court-geometry constraints** (reprojection loss, metric root, foot-on-ground) — free accuracy.
8. **Foot-skate killer + physics** (contact-vs-Z=0 lock + CCD-IK → PhysPT) — the watchable, skate-free replay (`IMPLEMENTATION_PHASES.md` Phase 4).
9. **Racket 6DoF** — paddle-face + contact-point, gated behind true-corner/CAD/reference evidence and RKT evaluation before any face-angle claim.
10. **Velocity pipeline + Protocol A/B/C validation** — turn on defensible velocity claims.
11. **Unified 2-camera shoot + auto-labeling loop; multi-view→monocular distillation** — manufacture ground truth; the accuracy moat.
12. **Shot classification + corrections flywheel** — durable, compounding moat.
