...
**WIND-DOWN STATE (2026-07-05 evening):** all sessions closed; A100 spot VM terminated; next session starts from `RESET_HANDOFF_20260705.md` (repo root — the canonical restart doc). Final speed state: Wolverine E2E 2141s → 1144s (slim monoliths) → **702.4s committed default** (chunkfix, `runs/body_chunkfix_verify_20260705T204618Z`) → **473s BODY stage wall / ≈533s E2E with the opt-in array-native path** (`runs/body_payload_collapse_verify_20260705T221027Z`, lock-wait confound backed out). Array-native stays OPT-IN OFF: the isolation diff proved it drops stance lower-body smoothing protection (828→0 protected frames, mm-scale joint deltas, gates unaffected) — root cause + fix booked in `runs/lanes/payload_collapse_isolation_20260705/REPORT.md`. VISUAL POLISH landed + live-verified (`runs/visual1_wolverine_20260705T220517Z`, 532.3s E2E, gates green): smoothing resets 14→2, feet jitter RMS −60–75%, root-step p95 0.267→0.100; VP-C2 mesh-root fix + mesh index in all previews + 2× FPS button landed. BALL 3D chain is the committed pipeline default (trail + honesty KPI + fail-closed viewer parsing); P3-A BVP solver committed as documented WIP (see RESET_HANDOFF §8).
...
4. **RESOLVED 2026-07-05:** mesh index is now built by default (VM in-memory, 23s) and viewer-consumable on all previews; monoliths are opt-in via `--fetch-body-monoliths`. Remaining mesh caveat: contact-dense (`ball_aware`) scheduling cannot use physical-contact triggers on a fresh default run because the default chain does not emit `events_selected.json` (wiring gap booked in `runs/lanes/wiring_audit_20260705/WIRING_TRUTH_TABLE.md`).
...
9. **BALL P3-A BVP solver is committed WIP:** test-green incl. frozen-config guard, fail-closed-conservative, but 5 previously-good baseline intervals lose `fit` status on reselection and internal-val F1 was not rerun — finish-line steps in `runs/lanes/ball_p3a_bvp_anchor_first_20260705/report.json` `next`.
10. **Standing operational gotchas:** remote BODY fails exit 1 (pydantic extra_forbidden) if new tracks/placement fields ship without re-syncing `schemas/__init__.py` to the VM; Outdoor-length clips need `--remote-command-timeout-s 7200`.
...
*Last updated: 2026-07-05 (wind-down session; GPU terminated; see RESET_HANDOFF_20260705.md).*
