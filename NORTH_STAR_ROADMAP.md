...
| COACHING | Deterministic facts plus runner-enforced zero-fabrication audit before manifest (NS-05.1 core landed); **2026-07-26 rights-aware Coach Compare preview landed locally** with a strict comparison contract, exact clip/hash binding, five phase anchors, at most three evidence-linked cues, optional replay-manifest plumbing, and an editorial one-cue UI. The first indoor example compares measured BODY motion with a published dink rubric and links the official Ben Johns lesson unmodified; it does not fabricate pro joints, contact, or outcome causality | 182 focused viewer tests + 10 replay-manifest tests pass, strict TypeScript and production build pass; this is a `PARTIAL` motion-comparison preview, not a verified coach. No permissioned professional reference motion, trusted ball/paddle contact, expert audit, owner audit, or independent improvement evidence exists | Capture or license true 60/120 FPS professional/qualified-coach reference motion with likeness and derivative rights; process it through the same BODY path, review phase anchors, add BALL/RKT-gated contact facts, then pass reference-faithfulness, owner, and ≥4.0-player audits with zero fabricated numbers |
