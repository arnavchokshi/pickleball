...
- training the event head (or any model) on pb.vision gallery predictions: competitor terms restrict automated/commercial/competing use, PB+its own outputs are one teacher not independent votes, and training on the gallery destroys the head-to-head benchmark. The 13 harvested PB source games (12 gallery + Demo Vid, IDs pinned in data/pbvision_gallery_20260719/MANIFEST.json) are PERMANENTLY compare-only; pseudo-labels (if ever) come from rights-cleared public/owned pixels + our own predictors, never PB — reopen needs written training rights + a frozen source-disjoint control + ≥47/50 audited precision (advisory 2026-07-20 D3);
...
| 3 | **Event labeling — DONE for now, do NOT continue.** 102 rows banked 2026-07-19 (60 typed + 42 neg; 61 train / 41 val), `data/event_labels_owner_20260719/`. The first pickleball fine-tune runs on these. | Advisory 2026-07-20 D4: only ask for one 50-row uncertainty round IF the first fine-tune shows median G_val ≥ +0.10 but stays < 0.80 macro-F1; else stop. Fresh owner-shot footage outranks another old-pack round. | Fine-tune proceeds on 102; no more owner label time unless the measured gate fires. |
