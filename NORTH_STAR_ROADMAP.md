...
| P0-G | Post-BODY refined events/arcs still hidden inside `world` (explicitization HELD for the live ball_arc guard lane); 2026-07-16 `evidence17` landed: audio soft evidence now modulates normal-path fusion (bounded, non-gating), BOTH IPPE poses survive with ambiguity flags (collapse fixed), repaired confidence explicitly marked; size/diameter depth still unused and dependency-hashing hunks are booked-not-applied | Diameter evidence and refined-stage timing still cannot improve contacts/flight measurably; independent-error proof absent. | Explicit timed refined stages; audio/diameter affect independent error; both IPPE poses and corrected confidence survive to fusion (pose/confidence slices now wired at scoped-pass level; independent-error clause open). |
...
|---:|---|---|---|
| 1 | **NS-01.6/01.7 remainder** — most of the row landed 2026-07-16 (one authoritative stage graph, legacy pipeline_cli deleted, typed expected-optional failures w/ loud unexpected-exception semantics, frame-schedule completeness, P0-H representation/transform slices, audio-soft/both-IPPE/repaired-confidence plumbing; trackC rulings runs/manager/trackC_20260716/RULINGS.md). Remaining: make post-BODY refined events/arc explicit timed stages (~122s hidden in `world`; HELD for the ball_arc guard lane ruling), apply the booked contact-dependency-hashing runner hunks, ball size/diameter into events (needs runner blur-sidecar generation), modality ablations | Explicit timed stages + hashing + diameter wire; no promotion claim | Cold/reuse/partial/failure tests plus modality ablations |
