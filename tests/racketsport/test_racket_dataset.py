...
def test_validate_racket_dataset_accepts_valid_mixed_sources(tmp_path):
    ...
    (tmp_path / "racketvision" / "rv_clip.json").write_text(
        json.dumps({"frames": [{"corners": [[0, 0], [1, 0], [1, 1], [0, 1]]}]}),
        encoding="utf-8",
    )
    (tmp_path / "synthetic" / "blend_pose.json").write_text(
        json.dumps({"pose_labels": [{"rvec": [0, 0, 0], "tvec": [0, 0, 1]}]}),
        encoding="utf-8",
    )
    (tmp_path / "synthetic" / "blend_mask.png").write_bytes(_png_bytes())
    (tmp_path / "aruco" / "trial_001.json").write_text(
        json.dumps({"marker_detections": [{"id": 1, "corners": [[0, 0], [1, 0], [1, 1], [0, 1]]}]}),
        encoding="utf-8",
    )
...
def test_validate_racket_dataset_does_not_mark_placeholder_sources_ready(tmp_path):
    (tmp_path / "racketvision").mkdir()
    (tmp_path / "synthetic").mkdir()
    (tmp_path / "aruco").mkdir()
    ...
    manifest = _write_manifest(
        ...
        {
            "schema_version": 1,
            "dataset_id": "placeholder_racket_sources",
            "sources": [
                _entry("rv_clip", "racketvision", "racketvision/rv_clip.json", split="train", annotations=_annotations()),
                _entry(
                    "blend_pose",
                    "synthetic_blenderproc",
                    "synthetic/blend_pose.json",
                ...
                _entry("heldout_rv", "racketvision", "racketvision/rv_clip.json", split="test", annotations=_annotations()),
            ],
        },
    )
    ...
    assert payload["content_gaps"] == ["sources contain placeholder JSON only: 4", "segmentation masks contain placeholder bytes: 1"]
