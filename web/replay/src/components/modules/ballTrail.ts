...
export type BallHudState = "measured" | "predicted" | "not_visible";
...
export type BallHudReadout = {
  state: BallHudState;
  label: "ball: measured" | "ball: predicted" | "ball: not visible";
  lowConfidence: boolean;
...
export function parseBallTrailArtifact(input: unknown): BallTrailArtifact {
  ...
  return {
    samples: frames.map((frame, index) => readBallTrailSample(frame, index)),
    segments:
