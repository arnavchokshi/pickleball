...
<section id="reviewScreen" class="card hidden"><div id="vidwrap"><video id="player" playsinline></video><div id="marker"></div></div><div id="question"></div><div id="hint"></div>
...
function phase1(){{pending=null;click=null;marker.style.display="none";$("phase1btns").classList.remove("hidden");$("phase2btns").classList.add("hidden");$("question").textContent="What happened at the center candidate?";$("hint").textContent="It loops — watch with sound. Keys 1–5 select a decision.";player.loop=true;player.currentTime=0;player.play().catch(()=>{{}})}}
...
function render(){{if(i>=ITEMS.length){{finish();return}}show("reviewScreen");$("progress").textContent=(i+1)+" of "+ITEMS.length;player.src=ITEMS[i].file;$("back").style.visibility=i?"visible":"hidden";player.onloadedmetadata=()=>{{if(!pending){{player.loop=true;player.currentTime=0;player.play().catch(()=>{{}})}}else{{centerPaused()}}}};phase1()}}
function commit(decision,withClick){{const item=ITEMS[i],rec={{label_id:item.label_id,decision}};if(withClick){{if(!click)return;player.pause();rec.x=click.x;rec.y=click.y;rec.dt=Math.round(((player.currentTime-player.duration/2)/2)*10000)/10000}}answers[item.row]=rec;persist();i++;render()}}
player.addEventListener("click",e=>{{if(!pending)return;player.pause();const r=player.getBoundingClientRect();click={{x:Math.round((e.clientX-r.left)/r.width*10000)/10000,y:Math.round((e.clientY-r.top)/r.height*10000)/10000}};marker.style.left=(e.clientX-r.left)+"px";marker.style.top=(e.clientY-r.top)+"px";marker.style.display="block";$("confirm").disabled=false}});
