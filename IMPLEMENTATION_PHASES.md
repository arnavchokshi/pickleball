...
| iOS client | The Swift package has scaffold/unit-test coverage, and IOS-1 has partial AVFoundation preview/recording runtime code. The Xcode app target/device/simulator build is not verified in this checkout; sidecar writing, live-frame feed, fps/shutter/luminance measurement, codec validation, and physical-device gates are missing; IOS-2..IOS-6 remain scaffold-level | implement the missing AVFoundation/ARKit/Vision/CoreML/RealityKit/Upload runtime paths on a device and pass C1-C3 gates |
